...
for(year in 15:19){
  ...
  child_filename = paste0("senegal", year, ".DTA")
...
t = table(table(HH.Data$cid))                                                   
value = as.numeric(t)
category = as.numeric(names(t))
data = data.frame(category = category, value = value)

data = data %>%
  mutate(perc = value / sum(value)) %>%
  mutate(labels = scales::percent(perc, accuracy = 0.1)) %>%
  mutate(freq_perc = paste0(value, " (", labels, ")")) %>%
  mutate(perc_freq = paste0(labels, " (n=", value, ")"))

ggplot(data = data, aes(x = category, y = value)) + 
  geom_bar(stat = "identity", fill = "#4B9CD3", col = "black") + 
  geom_text(aes(label=freq_perc), size = 2.5, vjust = -0.5, col = "#4B9CD3") +
  labs(y = "Number of clusters") + 
  scale_x_discrete(name = "Cluster size (number of households)", limits = 2:12) + 
...
result <- HH.Data %>%
  group_by(cid) %>%
  summarize(across(-hhid, mean, .names = "{.col}"))
...
result_long <- result %>%
  gather(variable, value, -cid)

result_long = result_long %>% 
  mutate(variable = factor(variable, 
                           levels = c("Y", "A", "X_HHsize", "X_HHchildsize", 
                                      "X_job", "X_edu", "X_resp_age", "X_child_age", 
                                      "C_urban", "C_clsize" ),
                           ordered = T))

# Generate boxplots without data points and facet_wrap with fancy styling
ggplot(result_long, aes(x = variable, y = value)) +
