...
#**********           under CIPS varying delta policy                 **********	
#**********                                                           **********
#**********           Written by:				                              **********
#**********                                                           **********
#**********           Version: 2.0                                    **********
...
DGP <- function(N, type = "data", deltas = NULL, r = NULL){
  ...
  if(type == "estimands"){
    ...
    for(i in seq_len(length(deltas))){
      ...
      estimands[i, ] = c(deltas[i], mu, mu_1, mu_0)
...
estimator.simul <- function(dat, X.trt, X.out, deltas, nsplits, r){
  ...
  for (split in 1:nsplits) {
    ...
    for(i in which(fold == split)){
      ...
      deltas.i = deltas*(1+1/N[i])   ### cluster-size vary delta
      ...
      pi.delta.sub.par <- pi.sub.par %o% deltas.i / (pi.sub.par %o% deltas.i + 1 - pi.sub.par)       # N[i] x length(deltas.i) matrix
      
      cluster.prob.ratio.par <- deltas.i^(sum(dat.sub$A)) / 
        apply(pi.sub.par %o% deltas.i + 1 - pi.sub.par, 2, prod)                           # length(deltas.i) vector
      
      pi.delta.sub.rep.par <- rep(1, r) %x% pi.delta.sub.par                             # rN[i] x length(deltas.i) matrix (replicate pi.delta.sub r times)
      
      cluster.prob.par <- agg.fun(a*pi.delta.sub.rep.par + (1-a)*(1-pi.delta.sub.rep.par), prod)   # r x length(deltas.i) matrix
      
      add.factor.par <-
        (2*a-1) %o% deltas.i * rep(dat.sub$A - pi.sub.par, r) / 
        (rep(1, r) %x% (pi.sub.par %o% deltas.i + 1 - pi.sub.par)) /
        (a*rep(pi.sub.par, r) %o% deltas.i + (1-a)*(1-rep(pi.sub.par, r)))                 # rN[i] x length(deltas.i) matrix
      
      out.reg.mu.par <- apply(agg.fun(mu.a.par, mean) * (1 + agg.fun(add.factor.par, sum)) * 
                                2^(N[i]) * cluster.prob.par, 2, mean)                    # length(deltas.i) vector
      
      out.reg.mu_1.par <-
        apply(agg.fun( (mu.a.par * a) %o% rep(1, length(deltas.i)) / pi.delta.sub.rep.par * 
                         (1 + as.matrix(agg.fun(add.factor.par, sum)) %x% rep(1,N[i]) - add.factor.par), mean) *
                2^(N[i]) * cluster.prob.par, 2, mean)                                 # length(deltas.i) vector
      
      out.reg.mu_0.par <-
        apply(agg.fun( (mu.a.par * (1-a)) %o% rep(1, length(deltas.i)) / (1-pi.delta.sub.rep.par) * 
                         (1 + as.matrix(agg.fun(add.factor.par, sum)) %x% rep(1,N[i]) - add.factor.par), mean) *
                2^(N[i]) * cluster.prob.par, 2, mean)                                # length(deltas.i) vector
      
      ## Bias correction part for mu
      bias.cor.mu.par <- mean(dat.sub$Y - mu.sub.par, na.rm = T) * cluster.prob.ratio.par    # length(deltas.i) vector
      
      ## Bias correction part for mu_1
      bias.cor.mu_1.par <-
        apply( (dat.sub$Y - mu.sub.par) * I(dat.sub$A==1) / pi.delta.sub.par, 2, mean) * 
        cluster.prob.ratio.par                                                       # length(deltas.i) vector
      ...
      pi.delta.sub.nonpar <- pi.sub.nonpar %o% deltas.i / (pi.sub.nonpar %o% deltas.i + 1 - pi.sub.nonpar)       # N[i] x length(deltas.i) matrix
      
      cluster.prob.ratio.nonpar <- deltas.i^(sum(dat.sub$A)) / 
        apply(pi.sub.nonpar %o% deltas.i + 1 - pi.sub.nonpar, 2, prod)                           # length(deltas.i) vector
      
      pi.delta.sub.rep.nonpar <- rep(1, r) %x% pi.delta.sub.nonpar                             # rN[i] x length(deltas.i) matrix (replicate pi.delta.sub r times)
      
      cluster.prob.nonpar <- agg.fun(a*pi.delta.sub.rep.nonpar + (1-a)*(1-pi.delta.sub.rep.nonpar), prod)   # r x length(deltas.i) matrix
      
      add.factor.nonpar <-
        (2*a-1) %o% deltas.i * rep(dat.sub$A - pi.sub.nonpar, r) / 
        (rep(1, r) %x% (pi.sub.nonpar %o% deltas.i + 1 - pi.sub.nonpar)) /
        (a*rep(pi.sub.nonpar, r) %o% deltas.i + (1-a)*(1-rep(pi.sub.nonpar, r)))                 # rN[i] x length(deltas.i) matrix
      
      out.reg.mu.nonpar <- apply(agg.fun(mu.a.nonpar, mean) * (1 + agg.fun(add.factor.nonpar, sum)) * 
                                   2^(N[i]) * cluster.prob.nonpar, 2, mean)                    # length(deltas.i) vector
      
      out.reg.mu_1.nonpar <-
        apply(agg.fun( (mu.a.nonpar * a)[,rep(1, length(deltas.i))] / pi.delta.sub.rep.nonpar * 
                         (1 + as.matrix(agg.fun(add.factor.nonpar, sum)) %x% rep(1,N[i]) - add.factor.nonpar), mean) *
                2^(N[i]) * cluster.prob.nonpar, 2, mean)                                 # length(deltas.i) vector
      
      out.reg.mu_0.nonpar <-
        apply(agg.fun( (mu.a.nonpar * (1-a))[,rep(1, length(deltas.i))] / (1-pi.delta.sub.rep.nonpar) * 
                         (1 + as.matrix(agg.fun(add.factor.nonpar, sum)) %x% rep(1,N[i]) - add.factor.nonpar), mean) *
                2^(N[i]) * cluster.prob.nonpar, 2, mean)                                # length(deltas.i) vector
      
      ## Bias correction part for mu
      bias.cor.mu.nonpar <- mean(dat.sub$Y - mu.sub.nonpar, na.rm = T) * cluster.prob.ratio.nonpar    # length(deltas.i) vector
      
      ## Bias correction part for mu_1
      bias.cor.mu_1.nonpar <-
        apply( (dat.sub$Y - mu.sub.nonpar) * I(dat.sub$A==1) / pi.delta.sub.nonpar, 2, mean) * 
        cluster.prob.ratio.nonpar                                                       # length(deltas.i) vector
...
estimator <- function(dat, X.trt, X.out, deltas, nsplits, r){
  ...
  for (split in 1:nsplits) {
    ...
    for(i in which(fold == split)){
      ...
      pi.delta.sub <- pi.sub %o% deltas.i / (pi.sub %o% deltas.i + 1 - pi.sub)       # N[i] x length(deltas.i) matrix
      
      cluster.prob.ratio <- deltas.i^(sum(dat.sub$A)) / 
        apply(pi.sub %o% deltas.i + 1 - pi.sub, 2, prod)                           # length(deltas.i) vector
      
      pi.delta.sub.rep <- rep(1, r) %x% pi.delta.sub                             # rN[i] x length(deltas.i) matrix (replicate pi.delta.sub r times)
      
      cluster.prob <- agg.fun(a*pi.delta.sub.rep + (1-a)*(1-pi.delta.sub.rep), prod)   # r x length(deltas.i) matrix
      
      add.factor <-
        (2*a-1) %o% deltas.i * rep(dat.sub$A - pi.sub, r) / 
        (rep(1, r) %x% (pi.sub %o% deltas.i + 1 - pi.sub)) /
        (a*rep(pi.sub, r) %o% deltas.i + (1-a)*(1-rep(pi.sub, r)))                 # rN[i] x length(deltas.i) matrix
      
      out.reg.mu <- apply(agg.fun(mu.a, mean) * (1 + agg.fun(add.factor, sum)) * 
                                   2^(N[i]) * cluster.prob, 2, mean)                    # length(deltas.i) vector
      
      out.reg.mu_1 <-
        apply(agg.fun( (mu.a * a)[,rep(1, length(deltas.i))] / pi.delta.sub.rep * 
                         (1 + as.matrix(agg.fun(add.factor, sum)) %x% rep(1,N[i]) - add.factor), mean) *
                2^(N[i]) * cluster.prob, 2, mean)                                 # length(deltas.i) vector
      
      out.reg.mu_0 <-
        apply(agg.fun( (mu.a * (1-a))[,rep(1, length(deltas.i))] / (1-pi.delta.sub.rep) * 
                         (1 + as.matrix(agg.fun(add.factor, sum)) %x% rep(1,N[i]) - add.factor), mean) *
                2^(N[i]) * cluster.prob, 2, mean)                                # length(deltas.i) vector
      
      ## Bias correction part for mu
      bias.cor.mu <- mean(dat.sub$Y - mu.sub, na.rm = T) * cluster.prob.ratio    # length(deltas.i) vector
      
      ## Bias correction part for mu_1
      bias.cor.mu_1 <-
        apply( (dat.sub$Y - mu.sub) * I(dat.sub$A==1) / pi.delta.sub, 2, mean) * 
        cluster.prob.ratio                                                       # length(deltas.i) vector
